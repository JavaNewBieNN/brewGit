...
def send_test_report_email():
    try:
        ...
        with open('logs/pytest.log', 'rb') as log_file, open('report.tar.gz', 'rb') as tar_file:
            msg.add_attachment(log_file.read(), maintype='text', subtype='plain', filename='pytest.log')
            msg.add_attachment(tar_file.read(), maintype='application', subtype='gzip', filename='report.tar.gz')
